...
db-load-fixtures: wait-db db-drop db-create
	...
	@$(EXEC_DB) "mysql --user=root --password=root < /home/app/dump/eda_country.sql"
	@$(EXEC_MONGODB) mongorestore --drop --db eda_country /home/app/dump/documents/eda_country

## Load database from dump test
db-load-fixtures-test: wait-db db-drop-test db-create-test
	@echo "\nLoading fixtures from dump...\e[0m"
	@$(EXEC_DB) "mysql --user=root --password=root < /home/app/dump/eda_country_test.sql"
	@$(EXEC_MONGODB) mongorestore --drop --db eda_country_test /home/app/dump/documents/eda_country_test
...
db-reload-fixtures: env-dev wait-db db-reload-schema
	@$(EXEC_SYMFONY) doctrine:mongodb:schema:drop --db
	@$(EXEC_SYMFONY) doctrine:mongodb:schema:create
	@$(MAKE) import-country

	@echo "\nCreating dump...\e[0m"
	@$(EXEC_DB) "mysqldump --user=root --password=root --databases eda_country > /home/app/dump/eda_country.sql"
	@$(EXEC_MONGODB) mongodump --db eda_country --out /home/app/dump/documents
...
db-reload-fixtures-test: env-test wait-db db-reload-schema-test
	...
	@echo "\nCreating dump...\e[0m"
	@$(EXEC_DB) "mysqldump --user=root --password=root --databases eda_country_test > /home/app/dump/eda_country_test.sql"
	@$(EXEC_MONGODB) mongodump --db eda_country_test --out /home/app/dump/documents
...
code-quality: security-checker yaml-linter xliff-linter container-linter phpstan deptrac rector cs db-validate
